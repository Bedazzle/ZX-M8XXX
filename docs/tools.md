...
- MDR cross-format copy: extracting from MDR strips the 9-byte header and passes file type/address/autostart to the destination; copying to MDR constructs the header from source metadata. `mdrEditorParseHeader()` shared helper parses the header for both File Info and Edit tabs. MDR export (bin and Hobeta) strips the 9-byte header so the extracted file contains only payload data; Hobeta export takes the length/type/load-address from that header (via `hobetaSourceFile`).
...
- TZX Turbo Speed Data ($11) blocks whose pulse timings match the ROM loader (pilot 2168, sync 667/735, bits 855/1710) load at normal speed despite the turbo block format — common for the BASIC loader in modern re-releases (DeciLoad etc.). They're labelled "Standard Speed Data" (with `ID: $11` still shown in the meta line); blocks with non-standard timing keep the "Turbo Speed Data" label.
- In the Edit tab those standard-timed $11 blocks (when their payload is a ROM-format header/data) are parsed as first-class header/data files: selectable, editable, and copyable to other media (TAP/TRD/SCL/MGT/DSK), exactly like $10 blocks. `editorImportTzxData` keeps `tzxId: 0x11` + the timing params on the block and `editorBuildTzx` re-emits it as $11, so re-saving the source TZX is byte-lossless.
- Genuine turbo $11 blocks (non-standard timing or non-ROM payload) and pure-data $14 blocks stay opaque (`blockType: 'nonstandard'`) but are **listed in the Edit tab** as selectable `Turbo`/`Pure data` rows (flag byte + size). `editorRenderBlockList` shows them (only true metadata/control blocks — pause/text/archive/group/etc. — stay hidden), and `extractFilesFromPanel` / `editorExtractRaw` / `editorExtractHobeta` copy or extract them as a **raw CODE file**: the encoded byte stream after the timing-parameter header ($11: 18 bytes, $14: 10 bytes), with start address 0. The real load address/framing lives only in the game's own loader, so it's not guessed — copying raw turbo data is the user's call. (Direct-recording $15 blocks are pulse recordings, not byte streams, so they're not extractable this way.)
- **Turbo block timing — view / set / re-time** (the **Turbo** toolbar button, TZX panels only): opens dialog `#tzxTurboDialog` **pre-filled with the selected block's current pulse timing**, so a turbo block's parameters are visible (pilot/sync1/sync2/zero/one pulse lengths, pilot-tone pulse count, used-bits in last byte, pause). `editorReadTurboParams` reads them from `tzxTurboParams` (header/data) or the first 18 bytes of `rawBytes` (raw `$11`); a standard block shows standard ROM defaults (header pilot 8063, data 3223). Applying calls `editorConvertSelectionToTurbo`: for header/data blocks it sets `tzxId: 0x11` + an 18-byte `tzxTurboParams` (built by `editorBuildTurboParams`) so `editorBuildTzx` emits `$11`; for raw `$11` blocks it rewrites the 15-byte timing header in `rawBytes` in place (data length + data preserved). `editorBlockTurboable` gates which blocks qualify. Turbo-timed header/data rows are flagged with a ⚡ marker (`editorBlockIsTurbo`). Workflow: copy a file into a TZX (creates `$10` blocks via `editorAddFileBlocks`), then **Turbo** with the loader's timing — e.g. to swap a bug-fixed block into a game and load it with the original loader; or select an existing turbo block to inspect / change its speed.
...
- **Packed screen preview (ZX0/ZX7)**: when no *uncompressed* screen is found, the preview falls back to `findPackedScreenInBlock(bytes, loadAddr)` (`core/depackers.js`), which tries two strategies:
  - **Standalone** — `detectPackedScreen()` on a size-windowed block (`256 < len < 6912` — a simple screen can pack to well under 1 KB): speculatively depacks ZX0/ZX7, forward and backward (exit on first match).
  - **Embedded** — for larger blocks, `findDepackerSignature()` looks for the **standard ZX0/ZX7 depacker code** (position-independent inner-loop bytes: ZX7 `87 C0 7E 23 17 C9`, ZX0 `87 20 03 7E 23 17 D8`), then `locatePackedScreen()` finds the data **via the loader's own `LD HL,nnnn` source pointers** (mapped to a file offset through the block's `loadAddr`) — not by brute-force scanning. The `loadAddr` is threaded from tape headers, the Hobeta header, and disk catalog entries.
  - Either way the result is accepted only if it is **exactly 6912 bytes** with **low attribute-area entropy** (≤ 7.0, Shannon over the 768 attribute bytes) — exact length comes free from the self-terminating streams; the entropy guard rejects mis-depacked garbage. The depackers are output-capped (and reject impossible back-references) so a bad guess can't run away.
  - **RCS** (`rcsToScr`): RCS permutes only the bitmap (attributes stay put), so detection still fires. The de-scramble is **auto-detected by image coherence** — `looksRcsEncoded`/`screenCoherence` compare vertically-adjacent screen rows; de-RCS'ing a real RCS screen makes it markedly smoother, so the **RCS** checkbox is set automatically (manual override preserved; margin biased toward off so a non-RCS screen isn't wrongly scrambled). The raw decode is cached so toggling doesn't re-depack. (This coherence test made RCS-variant *signatures* unnecessary.)
  - On a hit the screen renders via `explorerRenderSCR`, labeled e.g. *"ZX7 packed screen @3520 [sig] — DMSCODE"* (offset + `[sig]` when signature-located, `(reverse)` for backward, `+RCS` when de-scrambled). Covers TAP/TZX, Hobeta, raw files, and TRD/SCL/MGT/MDR/OPD/Didaktik/DSK catalogs.
  - **Catalog marking & navigation** (`explorerMarkPackedScreens`): after the catalog paints, every previewable entry is flagged with an icon — **🖼️** screen / **🔤** font (uncompressed) and **📦** packed — so you can see and switch between several screens on one medium. Both go in the same spot uncompressed screens already use: disk's `.explorer-file-preview` cell, and tape's `.explorer-tap-preview` span after "Data length" (so packed and uncompressed icons share one place per medium). Clicking a marked entry previews it: packed hits are cached (no re-depack) and shown via the row click; uncompressed screens preview from **either the header or the data block** (the click handler resolves header↔data, so both behave the same — no more "header → hexdump, data → preview" split). Detection runs off the catalog's paint path so it never delays the listing. On tape (TAP/TZX) a data block that follows a header is indented under it to show the pairing.
  - **Save .scr**: a button on the packed-screen preview downloads the displayed 6912 bytes as `<name>.scr` — de-RCS'd when the **RCS** box is ticked, raw decode otherwise.
...
- Supported container formats: TAP, TZX, TRD, SCL, MGT, IMG, MDR, DSK, OPD, OPU, D40, D80, ZIP, SNA, Z80, SZX
...
- Toolbar: New (format dropdown), Save, Move Up/Down, Delete, Extract (bin / bin full-sectors / Hobeta dropdown), Copy (to other panel) with an optional **Keep slack** checkbox, Add File
- **New (create blank disk/tape)**: the dropdown creates an empty image for every editable format — TAP, TZX, TRD, SCL, MGT, MDR, OPD (SS 180K / DS 720K), Didaktik D40 (360K) / D80 (720K), the DSK variants (+3 SS/DS, CPC System/Data, Timex TOS), and ZIP. The disk-format blanks (`buildTRD`/`createBlankOPD`/`createBlankD40`/`createBlankD80`/`createBlankDSK`, etc.) reproduce real on-disk formats so the images are accepted by genuine tools — verified byte-for-byte in `tests/pristine-test.html` against externally-formatted reference blanks. (Snapshots have no "New": a blank memory dump is meaningless.)
- **Add File recognizes Hobeta for all formats**: a `.$X` file's 17-byte header is detected (`isHobetaExt`/`parseHobeta`) and stripped, with name/type/start prefilled into the add dialog (TR-DOS ext for disk formats, +3DOS type/ext for DSK, tape type for TAP/TZX). Non-Hobeta files are added as-is.
- **BASIC autostart on Add File (TAP/TZX)**: the add dialog shows the autostart line field when the file type is BASIC, so a start line can be entered manually (blank = no auto-run, header param1 = `0x8000`). It goes into the tape header only — the Hobeta/disk on-disk format is untouched. The BASIC program length (param2) is also carried through, so the file isn't reported as having no variables.
- **Editable containers** (TAP/TZX/TRD/SCL/MGT/MDR/DSK/OPD/D40/D80): add, remove, reorder, rename files; inline edit via double-click; save produces a new file
- **Uniform panel layout (tape ≈ disk ≈ snapshot)**: every Edit-tab catalog — tape, disk (TRD/SCL/MGT/MDR/OPD/Didaktik/DSK), ZIP and snapshot — renders through one shared layout so any two panels read consistently:
  - **Labeled column header** (`editorColHeaderHtml(countLabel)`): each panel opens with a `# Name Flag Type Address Bytes` header row — plus a `Sectors` (TRD/SCL/MGT/MDR/OPD) or `Blocks` (DSK) column where the format tracks one — and every file row emits its cells in that order, using the same `file-name`/`file-flag`/`file-ext`/`file-addr`/`file-size`/`file-sectors` classes everywhere (DSK uses `file-plus3type`, MGT/MDR use `file-mgttype`, all sized to the same 5ch Type column). **Units live in the header**, so rows show bare numbers (e.g. `16384`, `30`) rather than repeating "bytes"/"sectors"/"blocks". The **Flag** column shows a tape block's flag byte (`$00` header / `$FF` data / custom); it's empty for disk and snapshot panels.
  - **Addresses**: load addresses read `decimal ($hex)` and BASIC autostarts read `LINE n`, consistently across disk *and* snapshot panels.
  - **Label strip** (`.editor-panel-labelbar`, `editorRenderLabelBar` / `editorApplyLabel`): the volume label is panel-level metadata, so it lives in a thin strip **between the panel title and the file table**, not as a table row. Formats that carry a label — **TR-DOS** (TRD/SCL, 8 chars), **Opus** (OPD, 10), **Didaktik** (D40/D80, 10), **Microdrive** (MDR cartridge name, 10) and **+3/DSK** (CP/M Plus disk label, 11) — show an editable `Label: "…"` chip; click it to edit inline, then Apply/Enter writes it back (TR-DOS/Opus/MDR rebuild the image; Didaktik writes in place via `DidaktikLoader.setDiskLabel`; DSK via `DSKLoader.setDiskLabel`, which adds/updates a CP/M `0x20` label entry — an empty label removes it). Formats with no label slot (tape, snapshot, MGT, ZIP) get an **empty strip** (the strip reserves height so both panels' tables stay row-aligned). The file table itself now holds only the column header + file rows. For +3/DSK the strip shows even on an **empty** disk, so a blank disk can be named before any files are added.
  - **Combined tape rows** (`editorRenderBlockList`, when **Link header** is on — the default): a header and the data block right after it whose payload length matches the header's declared length (`editorIsPairHeader`, length-aware) fold into **one file-style row**. **Media order is preserved** and rows are **numbered sequentially**; unmatched blocks stay in place — a lone header (no/length-mismatched data) and headerless/leftover data render as their own columnar rows (empty cells hold the column positions). Type labels use disk vocabulary (BASIC/Code/…). **Checksum and TZX pause are omitted** in edit mode, and **non-file TZX blocks** (Archive Info, Text Description, Pause/Stop, Group/Loop, …) are **hidden** in edit mode — they stay in `panel.blocks` and are written back on save (they remain visible in the read-only File Info tab).
  - **Aligned columns**: the editor font is monospace, so the columns have fixed character widths (index 3ch, name 11ch, flag 4ch, type 5ch, address 14ch, bytes 8ch, sectors/blocks 7ch) and the flag/address/bytes cells are always emitted (even empty) so columns line up under their headers across all row types and panels. Rows never wrap (`.editor-block-info` is `nowrap`); a too-narrow panel scrolls horizontally instead.
  - **Inline editor**: double-clicking a row opens one compact, format-agnostic editor — `[Name] [Type/Ext ▾] [Addr]` with placeholders instead of field labels, no size readout, and a single Apply button. Each format supplies only the fields it has (e.g. MDR has no address; DSK uses Name+Ext).
  - **Operations** act on the whole pair: selecting a combined row selects both blocks (`editorExpandPairs`), Copy/Extract pair header+data (`extractFilesFromPanel`), Delete removes both, double-click edits the header. Turning **Link header** off restores the raw per-block view (every header/data on its own row, also decluttered).
- **Reorder (Move Up/Down)** works on every editable format — TAP/TZX and TRD/SCL/MGT/MDR via the list-rebuild model (`editorMoveSelection`/`diskEditorMoveSelection`), OPD via the same path, Didaktik via `DidaktikLoader.swapDirEntries` (swaps directory entries in place), DSK/+3DOS via `dskEditorMoveSelection` (reorders CP/M directory entries, keeping each file's extents grouped). Reordering only changes catalog order, never file data/chains. To **insert a file mid-list**, Add (it lands at the end) then Move Up into place. *Not* shown for snapshots (fixed structural entries) or ZIP (unwrapped transparently)
- **Soft-delete / undelete vs delete-forever**: **Mark Del** toggles a `deleted` flag on the selected files — shown with a greyed, struck-through filename, kept in the list, and toggled off again to **undelete**. **Delete** removes forever (compacts). On save, **TR-DOS keeps** deleted entries on the disk (written with the `0x01` marker by `buildTRD`, recoverable); every other format **flushes** them from the written image (TAP/TZX/MGT/MDR/OPD rebuild from the non-deleted list; Didaktik `0xE5`s them in a copy via `DidaktikLoader.deleteFile`; DSK `0xE5`s them in a cloned `DSKImage`). For the in-place formats (Didaktik, DSK) the `deleted` flag is preserved across the re-list on refresh by a stable key (start sector / `user:name:ext`). The status line shows `(+N deleted, flushed on save)`.
- **Delete + add free-space reclamation** differs by allocation model: list-rebuild formats (TRD/SCL/MGT/MDR/OPD) splice the deleted file out and **repack on save** — so even OPD's contiguous allocation reclaims a mid-disk hole for a larger file; in-place formats free and re-allocate scattered units directly (Didaktik frees the FAT chain, DSK marks the CP/M entry `0xE5` and `getBlockAllocationMap` recomputes free blocks)
...
- **Keep last-sector slack (TR-DOS/SCL)**: a file fills whole 256-byte sectors, so its last sector usually has unused trailing bytes ("slack") that can hold hidden data. By default Copy and Extract use the *logical* length and drop the slack. Two opt-ins preserve it: the **Keep slack** checkbox carries a CODE file's slack on Copy (written into the destination entry's padded tail by `applyKeptSlack`), and **Extract → bin (full sectors)** dumps the whole `sectors×256` allocation (no header strip). These appear only when the panels are TR-DOS/SCL (`SLACK_FORMATS`), the formats whose editor model keeps full sectors (`f.data` = `sectors×256`); since both use 256-byte sectors the slack always fits exactly. For OPD/MGT/MDR/Didaktik/DSK the model stores logical data and pads only at build time, so slack carry there would need each writer extended (and `applyKeptSlack` already truncates to the destination's room for the future case where a target's sector size differs). SCL is an archive (its `sectors` byte is a 256-byte length unit, not disk geometry), but it shares the TR-DOS model path so trailing bytes still round-trip losslessly. BASIC files are excluded (the in-band autostart trailer makes the logical end ambiguous)
...
- **Didaktik D40/D80**: full read-write MDOS editor — Add File (dialog `B`=BASIC→MDOS `P`, `C`=Code→MDOS `B`), Delete (multi-select), inline Rename + LINE/load-address edit (double-click → Apply), reorder (Move Up/Down via `DidaktikLoader.swapDirEntries`), Save (`.d40`/`.d80`), plus Extract (Hobeta `.$X` or raw — multi-select → zip) and bidirectional Copy (Didaktik files out to the other pane, or files from any other format copied into the Didaktik disk via `addConvertedFile`). Edits are in-place on the image (boot sector + other files preserved; free space FAT-derived). P files → TAP BASIC (type 0, autostart from the catalog LINE), B files → TAP Code (type 3, load address from `startAddr`)
...
- Visual sector-level disk structure for DSK, TRD, MGT, OPD, MDR, and Didaktik D40/D80 images
...
- `buildMDRSectorMap()` — Microdrive cartridge sectors (radial map flagged `isMDR`; 254 sectors/cartridge)
- `buildDidaktikSectorMap()` — Didaktik MDOS allocation by walking each file's 12-bit FAT chain (sector 0 = boot, 1-5 = FAT, 6-13 = directory, 14+ = data)
- File colors: curated palette (`DISKMAP_FILE_PALETTE`) chosen to avoid the fixed sector-type colors (gray reserved, gold directory, red error, dark free/empty) so a file is never confused with a reserved/error sector; for disks with more files than palette entries the hue repeats with a ±22 lightness shift
