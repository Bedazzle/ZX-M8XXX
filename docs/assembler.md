...
- **Case** (select: UPPERCASE / lowercase / as-is) — folds only instructions, registers, conditions and directives; labels, identifiers, numbers and strings keep their case.
...
- **Byte** / **Word** (two selects: as-is / To hex / To decimal) — convert numeric literals' *base*, chosen **independently per value width**. A literal is a byte if its value ≤ `$FF`, else a word; each width picks its target base. So **Byte → To decimal, Word → To hex** keeps 8-bit immediates decimal while turning addresses into hex (`LD A,$FF` → `LD A,255`, `LD HL,$8000` stays `$8000`). Classification is by *value*, not source width, so `LD HL,$40` (=64) is treated as a byte. **To hex** emits in the Hex notation below (or `$`). Takes **precedence** over the Hex/Binary/Octal notation options. Value-preserving and shares all the safety rules: temp labels (`1b`/`2f`), the `$` current-address, `$+n`/`$-n`, the modulo operator, and string contents are left alone.
- **Pad byte hex to 2 digits** / **Pad word hex to 4 digits** (checkboxes) — pad hex *output* to a fixed width by value class: a byte value (≤ `$FF`) to 2 digits (`$5`→`$05`), a word value (`$100`–`$FFFF`) to 4 digits (`$100`→`$0100`); values over `$FFFF` are left as-is. "Byte"/"word" are the standard Z80 widths (`DB`/`DEFB`, `DW`/`DEFW`). Independent toggles, so byte padding never widens a word value and vice-versa. They apply wherever hex is emitted — both Base **To hex** conversion and Hex notation unification (so `$5`→`$05` even without a base change, provided a Hex notation is selected); the `h`-suffix guard-zero rule still applies on top (`10`→`0Ah` with byte padding).
- **Hex** (select: as-is / `#` / `$` / `0x` / `0FFh`) — unifies hex notation: `$FF`, `#FF`, `0xFF`, `0FFh` all become the chosen form. Converting to the `h` suffix adds a guard zero when the value starts with a hex letter (`$FF`→`0FFh`); converting away from it drops the guard zero. Bare `$` (current address) and `$+n`/`$-n` relative refs are left alone; hex inside strings is untouched.
- **Binary** (select: as-is / `%` / `0b` / `1010b`) — unifies binary notation. The `b` suffix form is padded to a minimum of 3 digits (`%10`→`010b`) because the assembler reads a 1–2 digit `Nb` as a temp-label reference, not binary. The modulo operator (`COUNT%10`, `5 % 2`) is never mistaken for a `%` binary literal.
- **Octal** (select: as-is / `77o` / `77q`) — unifies the octal suffix; sjasmplus has no octal prefix form.
