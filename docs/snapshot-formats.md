...
**Validation**: End >= Start enforced proactively during input (fields are clamped as the user types). Length clamped to minimum 1 and maximum `0x10000 - start`. Export also validates before download.
...
- **Format**: PNG (image), SCR (raw 6912 bytes), GIF (flash) -- 2-frame animated GIF with both flash phases
- **Size**: Screen only (256x192 crop) or With border (full display)
- **Zoom**: x1, x2, x3 -- output pixel scaling
- **Overlay**: Include overlay canvas (grid, etc.) in the capture
