...
## v0.15.10
- **Assembler: module-scope label fallback**: an unqualified label referenced inside a `MODULE` now falls back to the enclosing/global scope when it isn't a local member (the sjasmplus rule) — previously it failed with `Undefined symbol: module.NAME`. Fixes real multi-module projects (e.g. a Lode Runner disassembly that referenced global labels from inside `MODULE ig_v`/`dpub`).
- **Assembler: `LABELSLIST` directive**: `LABELSLIST "file.l"` now writes a labels file (the sjasmplus "unreal speccy" `PP:AAAA name` format, sorted by address) into the Generated files list, downloadable like `SAVESNA`/`SAVEBIN`. Previously failed with `Unknown instruction: LABELSLIST`.
- **Assembler: `TAPOUT`/`TAPEND` directives**: wrap the bytes emitted between them into a standard ZX tape block (default flag `0xFF`, XOR checksum) and write a `.tap` file; multiple pairs to one file append blocks. Byte framing matches sjasmplus and round-trips through the tape loader.
