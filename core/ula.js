    ...
    export class ULA {
        ...
        ulaplusWriteData(val, tStates) {
            ...
            if (reg < 64) {
                ...
                // If writing to entries 8-15 (CLUT 0 PAPER = border colors), trigger border rendering
                // Border uses entry 8 + borderColor, so any CLUT 0 PAPER write may affect the border
                if (reg >= 8 && reg <= 15 && this.ulaplus.paletteEnabled && this.beamRendering && tStates !== undefined) {
        ...
        getBorderColor32() {
            if (this.ulaplus.enabled && this.ulaplus.paletteEnabled) {
                // Border uses CLUT 0 PAPER entries (8-15), indexed by port $FE border color
                return this.ulaplus.palette32[8 + this.borderColor];
            }
            return this.palette32[this.borderColor];
        }

        // Get border color at specific T-state (for raster effects)
        // Looks up CLUT 0 PAPER entry at the given T-state if ULAplus is active
        getBorderColor32AtTState(tState) {
            if (this.ulaplus.enabled && this.ulaplus.paletteEnabled && this.hadPaletteChanges) {
                // Build palette at this T-state and return border entry
                this.getPalette32AtTState(tState, this.paletteTempLine);
                return this.paletteTempLine[8 + this.borderColor];
            }
        ...
        renderScanline(line) {
            ...
            // ULAplus: border uses CLUT 0 PAPER entries 8-15 (indexed by borderColor)
            ...
            if (screenLine < 0 || screenLine >= 192) {
                ...
                if (this.borderOnly) {
                    ...
                    const attrRowOffset = (y >> 3) << 5;
                    const attrAddr = 0x1800 + attrRowOffset;
